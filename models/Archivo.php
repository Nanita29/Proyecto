<?php
    class Archivo extends Conectar{
        ...
        public function listar_archivo($id_item,$id_estado){
            ...
            FROM  archivo, usuario WHERE  archivo.id_usuario = usuario.id_usuario AND archivo.id_item = $id_item AND archivo.id_tabla = $id_estado;";
            $sql=$conectar->prepare($sql);
            $sql->execute();
            return $resultado=$sql->fetchAll();
            print_r ($sql);
        ...
        public function getRuta($id_archivo){
            $conectar= parent::conexion();
            parent::set_names();
