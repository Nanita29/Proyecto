...
$(document).ready(function(){
    ...
    tabla=$('#unidad_data').dataTable({
        ...
        "ajax":{
            url: '../../controller/unidad_a.php?op=listar',
            type : "post",
            dataType : "json",						
            error: function(e){
...
function editar(id_unidad_a){
    ...
    $.post("../../controller/unidad_a.php?op=mostrar", {id_unidad_a : id_unidad_a}, function (data) {
        ...
        //estados 2
        if((data.e1_2)==2){
            $("#e1_2t").prop("checked", true);
        };
        if((data.e1_2)==1){
            $("#e1_2f").prop("checked", true);
        };

        if((data.e2_2)==2){
            $("#e2_2t").prop("checked", true);
        };
        if((data.e2_2)==1){
            $("#e2_2f").prop("checked", true);
        };

        if((data.e3_2)==2){
            $("#e3_2t").prop("checked", true);
        };
        if((data.e3_2)==1){
            $("#e3_2f").prop("checked", true);
        };

        if((data.e4_2)==2){
            $("#e4_2t").prop("checked", true);
        };
        if((data.e4_2)==1){
            $("#e4_2f").prop("checked", true);
        };
        $('#a_avance').val(data.a_avance);
        $('#a_avance2').val(data.a_avance2);
...
function archivo(id_unidad_a){
    ...
    $.post("../../controller/unidad_a.php?op=mostrar_p/archivo", {id_unidad_a : id_unidad_a}, function (data) {
        data = JSON.parse(data);
...
function archivo_data(id_unidad_a,id_estado){
    ...
    $.post("../../controller/unidad_a.php?op=mostrar_p/archivo_data", {id_unidad_a : id_unidad_a}, function (data) {
        data = JSON.parse(data);
...
function guardaryeditar(e){
   ...
   /*  $.ajax({
        url: "../../controller/unidad_a.php?op=guardaryeditar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function guardar_archivo(e){
    ...
    if ($('#nombre_ar').val()=='' || $('#descripcion_ar').val()=='' || $('#tipo_ar').val()==''|| $('#archivo').val()==''){
        ...
        swal({
          ...
          function(){
            $.ajax({
                url: "../../controller/unidad_a.php?op=guardar_archivo",
                type: "POST",
                data: formData,
                contentType: false,
                processData: false,
